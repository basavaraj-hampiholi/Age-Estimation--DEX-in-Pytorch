...
def main():
    ...
    # Print status of the model for every 10 images
    print_freq=10
   
    # Loading weight vector for weighted cross entropy loss
    df=pd.read_csv('/hampiholidata/Project/Datasets/imdb/Augment/IMDB_Wiki_Adience_Train_Undersample_Dist.csv')
    wghts= df['Weights']
    ...
    optimizer = torch.optim.Adam(model.parameters(), L_Rate, betas=(0.9, 0.999), eps=1e-08,             
                                weight_decay=1e-4)
                                                       
    cudnn.benchmark = True

    #Loading the data through csv and preparing dataset
    transformed_train_dataset = AgeEstimationDataset(csv_file='imdb/Augment/IMDB_Wiki_Adience_Train_Undersample.csv',
                                           root_dir='imdb/',
    ...
    transformed_valid_dataset = AgeEstimationDataset(csv_file='imdb/IMDB_Wiki_Adience_Val.csv',
                                           root_dir='imdb/',
                                           transform=transforms.Compose([
                                           Rescale(256),
                                           RandomCrop(224),                                           
...
def save_checkpoint(state, is_best, epoch, filename='checkpoint_imdb_final.pth.tar'):
    torch.save(state, filename)
    if is_best:
        shutil.copyfile(filename, 'imdb_best_final.pth.tar')
